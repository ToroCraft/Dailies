...
public class SpawnBaileyCommand {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands.literal("spawn").executes((x) -> {
            return spawnBailey(x.getSource());
        }).then(Commands.literal("bailey").executes((y) -> {
            return spawnBailey(y.getSource());
        })));

        dispatcher.register(Commands.literal("dailies").executes((x) -> {
            return listDailyQuests(x.getSource());
        }).then(Commands.literal("list").executes((y) -> {
                return listDailyQuests(y.getSource());
        })
        ).then(Commands.literal("accept").executes((z) -> {
            return 1;
        }).then(Commands.argument("Quest Number", new QuestNumberArgument()).executes((a) -> {
            return acceptQuest(a.getSource(), a.getArgument("Quest Number", Integer.class));
        }))
        ).then(Commands.literal("abandon").executes((z) -> {
            return 1;
        }).then(Commands.argument("Quest Number", new QuestNumberArgument()).executes((a) -> {
            return abandonQuest(a.getSource(), a.getArgument("Quest Number", Integer.class));
        }))));
    }
